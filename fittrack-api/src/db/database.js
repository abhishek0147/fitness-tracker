...
if (IS_VERCEL) {
  ...
  if (process.env.DATABASE_URL) {
    // ── Real persistent PostgreSQL (Supabase / Neon / Vercel Postgres) ──
    const { Pool } = require("pg");
    const pool = new Pool({ connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false } });
    pool.query(`
      CREATE TABLE IF NOT EXISTS users (id SERIAL PRIMARY KEY, name TEXT NOT NULL, email TEXT UNIQUE NOT NULL, password TEXT NOT NULL, bio TEXT DEFAULT '', avatar_url TEXT DEFAULT '', created_at TIMESTAMPTZ DEFAULT NOW());
      CREATE TABLE IF NOT EXISTS activities (id SERIAL PRIMARY KEY, user_id INTEGER NOT NULL, title TEXT NOT NULL, type TEXT NOT NULL, distance REAL DEFAULT 0, duration INTEGER NOT NULL, elevation_gain REAL DEFAULT 0, calories INTEGER DEFAULT 0, avg_heart_rate INTEGER DEFAULT 0, notes TEXT DEFAULT '', date TIMESTAMPTZ DEFAULT NOW(), created_at TIMESTAMPTZ DEFAULT NOW());
      CREATE TABLE IF NOT EXISTS kudos (id SERIAL PRIMARY KEY, activity_id INTEGER NOT NULL, user_id INTEGER NOT NULL, created_at TIMESTAMPTZ DEFAULT NOW(), UNIQUE(activity_id, user_id));
      CREATE TABLE IF NOT EXISTS follows (id SERIAL PRIMARY KEY, follower_id INTEGER NOT NULL, following_id INTEGER NOT NULL, created_at TIMESTAMPTZ DEFAULT NOW(), UNIQUE(follower_id, following_id));
      CREATE TABLE IF NOT EXISTS activity_routes (id SERIAL PRIMARY KEY, activity_id INTEGER NOT NULL, latitude REAL NOT NULL, longitude REAL NOT NULL, elevation REAL DEFAULT 0, distance_from_start REAL DEFAULT 0, timestamp TIMESTAMPTZ DEFAULT NOW(), created_at TIMESTAMPTZ DEFAULT NOW());
    ...
    module.exports = {
      ...
      prepare(sql) {
        const pg = toPg(sql);
        return {
          async get(...a)  { const {rows} = await pool.query(pg, a.flat()); return rows[0]||null; },
          async all(...a)  { const {rows} = await pool.query(pg, a.flat()); return rows; },
          async run(...a)  {
    ...
    // ── In-memory fallback (no DATABASE_URL) — data resets on cold start ──
    console.log("[DB] In-memory store (set DATABASE_URL for persistence)");
    ...
    function tbl(sql) { const m=sql.match(/(?:FROM|INTO|UPDATE|DELETE\s+FROM)\s+(\w+)/i); return m?m[1].toLowerCase():null; }
    function wMatch(row, sql, params) {
      const m=sql.match(/WHERE\s+(.+?)(?:\s+ORDER|\s+LIMIT|\s+GROUP|$)/is);
      if(!m) return true;
      const pairs=[...m[1].matchAll(/(\w+)\s*=\s*\?/gi)];
      return pairs.every((p,i)=>String(row[p[1]]??"").toLowerCase()===String(params[i]??"").toLowerCase());
    }
    function runSQL(sql, params) {
      sql=sql.trim(); const t=tbl(sql); const rows=t?T[t]:[];
      if(/^SELECT/i.test(sql)) {
        let out=rows.filter(r=>wMatch(r,sql,params));
        const ob=sql.match(/ORDER\s+BY\s+(?:\w+\.)?(\w+)(?:\s+(DESC|ASC))?/i);
        if(ob){const[,col,dir="ASC"]=ob;out.sort((a,b)=>{const c=(a[col]??"")<(b[col]??"")?-1:(a[col]??"")>(b[col]??"")?1:0;return dir.toUpperCase()==="DESC"?-c:c;});}
        const lm=sql.match(/LIMIT\s+(\d+)(?:\s+OFFSET\s+(\d+))?/i);
        if(lm) out=out.slice(+(lm[2]||0),+(lm[2]||0)+ +lm[1]);
        if(/COUNT\(\*\)/i.test(sql)&&!/\b(SUM|AVG|MAX)\s*\(/i.test(sql)){const alias=(sql.match(/COUNT\(\*\)\s+as\s+(\w+)/i)||[])[1]||"count";return[{[alias]:out.length}];}
        if(/\b(SUM|AVG|MAX)\s*\(/i.test(sql)){
          ...
          for(const[,fn,col,alias] of sql.matchAll(/(SUM|AVG|MAX)\(\s*(?:COALESCE\(\s*)?(\w+)[^)]*\)\s*(?:\))?\s+as\s+(\w+)/gi)){
            ...
            res[alias]=fn==="SUM"?vals.reduce((a,b)=>a+b,0):fn==="AVG"?(vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:0):vals.length?Math.max(...vals):0;
          }
          const cp=sql.match(/COUNT\(\*\)\s+as\s+(\w+)/i); if(cp)res[cp[1]]=out.length; return[res];
        }
        return out;
      }
      if(/^INSERT/i.test(sql)){
        const cm=sql.match(/\(([^)]+)\)\s*VALUES/i); if(!cm)return{changes:0,lastInsertRowid:0};
        const cols=cm[1].split(",").map(s=>s.trim()); const id=IDS[t]++; const row={id,created_at:ts()};
        cols.forEach((c,i)=>{row[c]=params[i]!==undefined?params[i]:null;});
        if(t==="users"&&rows.some(r=>r.email?.toLowerCase()===row.email?.toLowerCase())){const e=new Error("UNIQUE constraint failed: users.email");e.code="SQLITE_CONSTRAINT_UNIQUE";throw e;}
      ...
      if(/^DELETE/i.test(sql)){const before=rows.length;T[t]=rows.filter(r=>!wMatch(r,sql,params));return{changes:before-T[t].length,lastInsertRowid:0};}
  ...
  // ── Local: better-sqlite3 wrapped in async ──
  ...
  sqlite.prepare = (sql) => { const s=orig(sql); return { async get(...a){return s.get(...a);}, async all(...a){return s.all(...a);}, async run(...a){return s.run(...a);} }; };
