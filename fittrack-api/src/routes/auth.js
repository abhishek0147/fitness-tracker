...
router.post("/google", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      ...
      const result = await db.prepare(
        "INSERT INTO users (name, email, password, bio) VALUES (?, ?, ?, ?)"
      ).run(name || "Google User", email.toLowerCase(), hashed, "Joined via Google");
...
router.get("/me", authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({ ...user, stats: { total_activities: 0, total_distance: 0, total_duration: 0, total_calories: 0 } });
  } catch (err) {
    return res.status(500).json({ error: "Internal server error" });
  }
});
