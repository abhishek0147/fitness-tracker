...
router.post("/", authMiddleware, async (req, res) => {
  const { title, type, distance, duration, elevation_gain, calories, avg_heart_rate, notes, date } = req.body;
  ...
  try {
    const result = await db
      .prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `)
      .run(req.user.id, title, type, distance || 0, duration, elevation_gain || 0, calories || 0, avg_heart_rate || 0, notes || "", date || new Date().toISOString());
...
router.post("/:id/kudos", authMiddleware, async (req, res) => {
  try {
    ...
    if (existing) {
      await db.prepare("DELETE FROM kudos WHERE activity_id = ? AND user_id = ?").run(req.params.id, req.user.id);
      const count = await db.prepare("SELECT COUNT(*) as count FROM kudos WHERE activity_id = ?").get(req.params.id);
      return res.json({ message: "Kudos removed", kudos_count: count.count, has_kudoed: false });
    } else {
      await db.prepare("INSERT INTO kudos (activity_id, user_id) VALUES (?, ?)").run(req.params.id, req.user.id);
      const count = await db.prepare("SELECT COUNT(*) as count FROM kudos WHERE activity_id = ?").get(req.params.id);
